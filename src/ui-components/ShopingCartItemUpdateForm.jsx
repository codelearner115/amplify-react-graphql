...
export default function ShopingCartItemUpdateForm(props) {
  const {
    id,
    shopingCartItem,
    onSuccess,
  ...
  const resetStateValues = () => {
    const cleanValues = { ...initialValues, ...shopingCartItemRecord };
  ...
  const [shopingCartItemRecord, setShopingCartItemRecord] =
    React.useState(shopingCartItem);
  React.useEffect(() => {
    const queryData = async () => {
      const record = id
        ? await DataStore.query(ShopingCartItem, id)
        : shopingCartItem;
      setShopingCartItemRecord(record);
    };
    queryData();
  }, [id, shopingCartItem]);
  React.useEffect(resetStateValues, [shopingCartItemRecord]);
  const validations = {
    lastPrice: [],
    newPrice: [],
    quantity: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          const original = await DataStore.query(ShopingCartItem, id);
          await DataStore.save(
            ShopingCartItem.copyOf(original, (updated) => {
      ...
      {...rest}
      {...getOverrideProps(overrides, "ShopingCartItemUpdateForm")}
    >
      <TextField
        label="Last price"
        isRequired={false}
